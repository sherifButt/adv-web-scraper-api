...
export class ProxyManager {
  ...
  public reportProxyResult(proxy: ProxyInfo, success: boolean, responseTime?: number): void {
    ...
    // Update success rate
    currentProxy.successRate =
      currentProxy.successRate === undefined
        ? success
          ? 1
          : 0
    ...
    if (responseTime) {
      currentProxy.responseTime =
        currentProxy.responseTime === undefined
          ? responseTime
          : currentProxy.responseTime * 0.9 + responseTime * 0.1;
    }

    logger.debug(
      `Updated proxy ${proxy.host}:${
        proxy.port
      } - Success rate: ${currentProxy.successRate?.toFixed(2)}, Response time: ${
        currentProxy.responseTime
      }ms`
    );
  }
