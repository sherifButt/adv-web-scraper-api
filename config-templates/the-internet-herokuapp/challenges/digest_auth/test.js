...
async function testDigestAuthConfig() {
  logger.info('Starting Digest Auth config test...');
  ...
  try {
    ...
    // Basic validation (checks if flow completed - it should, as the failing step is optional)
    if (result.status === 'completed') {
      logger.info(
        'SUCCESS: Digest Auth test completed (goto step was optional and likely failed as expected).'
      );
    } else {
      // This path shouldn't ideally be reached if the optional step is the only one failing
      logger.error('FAILURE: Digest Auth test did not complete successfully.');
      console.error('Detailed Result:', result);
    }
  } catch (error) {
    logger.error(`Error executing Digest Auth flow: ${error.message}`);
    console.error(error);
  } finally {
    await page.close();
    ...
    logger.info('Digest Auth config test finished.');
  }
}

testDigestAuthConfig();
