...
async function testContextMenuConfig() {
  logger.info('Starting Context Menu config test...');
  ...
  try {
    ...
    if (result.status === 'completed') {
      logger.info('SUCCESS: Context Menu test completed successfully (dialog handled).');
    } else {
      logger.error('FAILURE: Context Menu test did not complete successfully.');
      console.error('Detailed Result:', result);
    }
  } catch (error) {
    logger.error(`Error executing Context Menu flow: ${error.message}`);
    console.error(error);
  } finally {
    await page.close();
    ...
    logger.info('Context Menu config test finished.');
  }
}
