...
async function testCheckboxesConfig() {
  logger.info('Starting Checkboxes config test...');
  ...
  try {
    ...
    if (result.status === 'completed') {
      logger.info('SUCCESS: Checkboxes test completed successfully.');
    } else {
      logger.error('FAILURE: Checkboxes test did not complete successfully.');
      console.error('Detailed Result:', result);
    }
  } catch (error) {
    logger.error(`Error executing Checkboxes flow: ${error.message}`);
    console.error(error);
  } finally {
    await page.close();
    ...
    logger.info('Checkboxes config test finished.');
  }
}

testCheckboxesConfig();
